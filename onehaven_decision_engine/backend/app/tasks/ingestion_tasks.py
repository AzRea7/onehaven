...
def sync_source_task(org_id: int, source_id: int, trigger_type: str = "scheduled"):
    db = SessionLocal()
    try:
        source = db.get(IngestionSource, int(source_id))
        if source is None or int(source.org_id) != int(org_id):
            return {"ok": False, "error": "source_not_found"}
        run = execute_source_sync(db, org_id=int(org_id), source=source, trigger_type=trigger_type)
