...
def sync_crawl_result_to_inventory(
    ...
    *,
    ...
    normalized_fetch.setdefault("current_fingerprint", fetch_result.get("current_fingerprint") or change_summary.get("current_fingerprint") or getattr(source, "current_fingerprint", None) or getattr(source, "content_sha256", None))
    normalized_fetch.setdefault("previous_fingerprint", fetch_result.get("previous_fingerprint") or change_summary.get("previous_fingerprint"))
    normalized_fetch.setdefault("comparison_state", fetch_result.get("comparison_state") or change_summary.get("comparison_state"))
    normalized_fetch.setdefault("change_kind", fetch_result.get("change_kind") or change_summary.get("change_kind"))
    normalized_fetch.setdefault("actionable_outcome", fetch_result.get("actionable_outcome") or change_summary.get("actionable_outcome"))
    normalized_fetch.setdefault("changed", bool(fetch_result.get("changed") or change_summary.get("changed")))
    normalized_fetch.setdefault("change_detected", bool(fetch_result.get("change_detected") or change_summary.get("change_detected") or normalized_fetch.get("changed")))
    normalized_fetch.setdefault("revalidation_required", bool(fetch_result.get("revalidation_required") or change_summary.get("requires_revalidation")))
    normalized_fetch.setdefault("raw_path", fetch_result.get("raw_path") or change_summary.get("raw_path") or getattr(source, "raw_path", None))
    normalized_fetch.setdefault("content_sha256", fetch_result.get("content_sha256") or change_summary.get("current_fingerprint") or getattr(source, "content_sha256", None))
    normalized_fetch.setdefault("retry_due_at", fetch_result.get("retry_due_at") or change_summary.get("retry_due_at") or getattr(source, "next_refresh_due_at", None))
