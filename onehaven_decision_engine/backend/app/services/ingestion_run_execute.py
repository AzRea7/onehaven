...
from ..models import Deal, Property, PropertyPhoto, RentAssumption, UnderwritingResult
...
def _normalize_property_type(value: Any) -> str:
    ...
    single_family_aliases = {
        "single_family", "singlefamily", "single_family_home",
        "single_family_residential", "house", "detached", "sfh", "residential",
    }
    multi_family_aliases = {
        "multi_family", "multifamily", "multi_family_home", "duplex", "triplex", "fourplex",
        "2_family", "3_family", "4_family",
    }
...
def _upsert_property(db: Session, *, org_id: int, payload: dict[str, Any]):
    existing = find_existing_property(
        db, org_id=org_id, address=payload["address"], city=payload["city"], state=payload["state"], zip_code=payload["zip"]
    )
    created = False
    if existing is None:
        existing = Property(
...
def _upsert_deal(db: Session, *, org_id: int, property_id: int, snapshot_id: int | None, payload: dict[str, Any]):
    existing = db.scalar(
        select(Deal).where(Deal.org_id == int(org_id), Deal.property_id == int(property_id)).order_by(desc(Deal.id))
    )
    created = False
    if existing is None:
        existing = Deal(
...
def _upsert_rent_assumption(db: Session, *, org_id: int, property_id: int, payload: dict[str, Any]):
    existing = db.scalar(
        select(RentAssumption).where(
            RentAssumption.org_id == int(org_id), RentAssumption.property_id == int(property_id)
    ...
    for p in photos:
        ...
        if existing is None:
            db.add(PropertyPhoto(
                org_id=int(org_id), property_id=int(property_id), url=url, kind=kind or "unknown", source=provider
            ))
            count += 1
    db.flush()
    return count
...
def _load_rows(source, trigger_type: str, runtime_config: dict[str, Any] | None = None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
...
def _is_valid_payload(payload: dict[str, Any]) -> bool:
    return bool(payload.get("external_record_id") and payload.get("address") and payload.get("city") and payload.get("state") and payload.get("zip"))
...
def _matches_runtime_filters(payload: dict[str, Any], runtime_config: dict[str, Any]) -> bool:
    ...
    if state and _norm_text(payload.get("state")) != _norm_text(state):
        return False
    if county and _norm_text(payload.get("county")) and _norm_text(payload.get("county")) != _norm_text(county):
        return False
...
def _write_underwriting_result(db: Session, *, org_id: int, deal_id: int, payload: dict[str, Any], rent_used: float):
    purchase_price = float(payload.get("asking_price") or 0)
    if purchase_price <= 0 or rent_used <= 0:
        return None
    ...
    result = underwrite(
        ...
        interest_rate=0.075,
        term_years=30,
        gross_rent=float(rent_used),
    )
    decision = "GOOD_DEAL"
    if result.dscr < 1.0 or result.cash_flow < 0:
        decision = "REJECT"
    elif result.dscr < 1.2 or result.cash_flow < 250:
        decision = "REVIEW"
    ...
    row = UnderwritingResult(
        ...
        deal_id=int(deal_id),
        decision=decision,
        score=max(1, min(100, int(round(result.dscr * 40 + max(result.cash_flow, 0) / 25)))),
        reasons_json=[
            f"Auto-underwritten on ingest.",
            f"DSCR {result.dscr:.2f}.",
            f"Cash flow ${result.cash_flow:.0f}/mo.",
        ],
        cash_flow=result.cash_flow,
        dscr=result.dscr,
        gross_rent_used=float(rent_used),
...
def _run_post_import_hooks(db: Session, *, org_id: int, property_id: int, deal_id: int | None, payload: dict[str, Any]) -> None:
    try:
    ...
    if deal_id is not None:
        try:
            rent_used = float(payload.get("approved_rent_ceiling") or payload.get("market_rent_estimate") or 0)
            _write_underwriting_result(db, org_id=org_id, deal_id=deal_id, payload=payload, rent_used=rent_used)
        except Exception:
            pass
...
def execute_source_sync(db: Session, *, org_id: int, source, trigger_type: str = "manual", runtime_config: dict[str, Any] | None = None):
    ...
    summary = {
        "records_seen": 0, "records_imported": 0, "properties_created": 0, "properties_updated": 0,
        "deals_created": 0, "deals_updated": 0, "rent_rows_upserted": 0, "photos_upserted": 0,
        "duplicates_skipped": 0, "invalid_rows": 0, "filtered_out": 0, "matched_before_limit": 0,
        "launch": normalized_runtime, "post_import_hooks_attempted": 0,
    }
    try:
        ...
        for raw_row in rows:
            ...
            fingerprint = build_property_fingerprint(
                address=payload["address"], city=payload["city"], state=payload["state"], zip_code=payload["zip"]
            )
        ...
        for raw_row in capped_rows:
            ...
            existing_link = find_existing_by_external_id(
                db, org_id=org_id, provider=source.provider, external_record_id=ext_id
            )
            ...
            deal, deal_created = _upsert_deal(db, org_id=org_id, property_id=prop.id, snapshot_id=None, payload=payload)
            _upsert_rent_assumption(db, org_id=org_id, property_id=prop.id, payload=payload)
            photo_count = _upsert_photos(
                db, org_id=org_id, property_id=prop.id, provider=source.provider, photos=payload.get("photos") or []
            )
            upsert_record_link(
                db, org_id=org_id, provider=source.provider, source_id=source.id, external_record_id=ext_id,
                external_url=payload.get("external_url"), property_id=prop.id, deal_id=deal.id if deal else None,
                raw_json=payload.get("raw") or raw_row, fingerprint=fingerprint,
            )
            ...
            _run_post_import_hooks(
                db, org_id=int(org_id), property_id=int(prop.id), deal_id=int(deal.id) if deal else None, payload=payload
            )
        ...
        db.rollback()
        return finish_run(
            db, row=run, status="failed", summary=summary, error_summary=str(e), error_json={"type": type(e).__name__}
        )
