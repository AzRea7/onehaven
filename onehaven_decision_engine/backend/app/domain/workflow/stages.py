...
_STAGE_META: dict[str, dict[str, str]] = {
    "discovered": {"label": "Discovered", "description": "Property exists in the system but has not been shortlisted into an active acquisition review.", "primary_action": "Save or shortlist property"},
    "shortlisted": {"label": "Shortlisted", "description": "Property is in the investor review queue and should be underwritten next.", "primary_action": "Run underwriting"},
    "underwritten": {"label": "Underwritten", "description": "Underwriting exists, but the property is not yet actively in offer / purchase execution.", "primary_action": "Decide whether to pursue"},
    "offer": {"label": "Offer", "description": "Property passed analysis and is now in active acquisition execution.", "primary_action": "Track offer / close status"},
    "acquired": {"label": "Acquired", "description": "Property has been purchased and is entering post-close operational setup.", "primary_action": "Start rehab/compliance setup"},
    "rehab": {"label": "Rehab", "description": "Rehab tasks exist and must be completed before compliance and leasing can move forward.", "primary_action": "Complete rehab tasks"},
    ...
    "tenant_screening": {"label": "Tenant Screening", "description": "Applicants are being reviewed, matched, or progressed toward lease execution.", "primary_action": "Screen and assign tenant"},
    "leased": {"label": "Leased", "description": "Lease is active and the unit is transitioning into stable occupancy.", "primary_action": "Prepare move-in / first month ops"},
    ...
    "turnover": {"label": "Turnover", "description": "A prior occupancy ended and the property needs either re-compliance or re-evaluation.", "primary_action": "Route turnover work"},
    "maintenance": {"label": "Maintenance", "description": "Property is in long-run management mode with maintenance and admin operations.", "primary_action": "Work maintenance / support queue"},
}
...
def stage_catalog() -> list[dict[str, Any]]:
    ...
    for idx, stage in enumerate(STAGES):
        ...
        rows.append({"key": stage, "rank": idx, "label": meta["label"], "description": meta["description"], "primary_action": meta["primary_action"]})
    return rows
...
def infer_transition_reason(previous_stage: Optional[str], current_stage: Optional[str]) -> Optional[str]:
    ...
    mapping = {
        ...
        ("underwritten", "offer"): "offer_ready",
        ("offer", "acquired"): "acquisition_complete",
        ("acquired", "rehab"): "rehab_started",
        ...
        ("turnover", "management"): "occupied_stabilized",
    }
...
def gate_for_next_stage(*, current_stage: str, decision_bucket: str, deal_exists: bool, underwriting_complete: bool, offer_ready: bool, acquired_complete: bool, rehab_complete: bool, inspection_exists: bool, compliance_complete: bool, lease_exists: bool, tenant_complete: bool, cash_complete: bool, occupied_complete: bool, turnover_active: bool = False) -> GateResult:
    ...
    if cur == "discovered":
        if not deal_exists:
            return GateResult(ok=False, blocked_reason="Property must be shortlisted before it can move deeper into the workflow.", allowed_next_stage="shortlisted", blockers=["not_shortlisted"])
        return GateResult(ok=True, allowed_next_stage="shortlisted")
    if cur == "shortlisted":
        if not underwriting_complete:
            return GateResult(ok=False, blocked_reason="Run underwriting before the property can move forward.", allowed_next_stage="underwritten", blockers=["missing_underwriting"])
        return GateResult(ok=True, allowed_next_stage="underwritten")
    if cur == "underwritten":
        if decision == "REJECT":
            return GateResult(ok=False, blocked_reason="Rejected properties cannot move into acquisition until assumptions change.", allowed_next_stage=None, blockers=["decision_reject"])
        if not offer_ready:
            return GateResult(ok=False, blocked_reason="The property is not yet ready for active acquisition execution.", allowed_next_stage="offer", blockers=["offer_not_ready"])
        return GateResult(ok=True, allowed_next_stage="offer")
    if cur == "offer":
        if not acquired_complete:
            return GateResult(ok=False, blocked_reason="Close the acquisition before moving into post-close workflow.", allowed_next_stage="acquired", blockers=["not_acquired"])
    ...
    if cur == "rehab":
        if not rehab_complete:
            return GateResult(ok=False, blocked_reason="Complete rehab tasks and clear blockers first.", allowed_next_stage="compliance_readying", blockers=["rehab_incomplete"])
        return GateResult(ok=True, allowed_next_stage="compliance_readying")
    if cur == "compliance_readying":
        if not inspection_exists:
            return GateResult(ok=False, blocked_reason="Create or schedule inspection before advancing.", allowed_next_stage="inspection_pending", blockers=["missing_inspection"])
        return GateResult(ok=True, allowed_next_stage="inspection_pending")
    if cur == "inspection_pending":
        if not compliance_complete:
            return GateResult(ok=False, blocked_reason="Pass inspection and clear compliance blockers first.", allowed_next_stage="tenant_marketing", blockers=["compliance_incomplete"])
        return GateResult(ok=True, allowed_next_stage="tenant_marketing")
    if cur == "tenant_marketing":
        if not lease_exists:
            return GateResult(ok=False, blocked_reason="Open tenant workflow and create a placement candidate before lease-up.", allowed_next_stage="tenant_screening", blockers=["no_tenant_progress"])
        return GateResult(ok=True, allowed_next_stage="tenant_screening")
    if cur == "tenant_screening":
        if not tenant_complete:
            return GateResult(ok=False, blocked_reason="An active lease is required before the unit becomes leased.", allowed_next_stage="leased", blockers=["lease_not_active"])
        return GateResult(ok=True, allowed_next_stage="leased")
    if cur == "leased":
        if not cash_complete:
            return GateResult(ok=False, blocked_reason="Cashflow records must start before the property can be treated as occupied.", allowed_next_stage="occupied", blockers=["cash_not_started"])
