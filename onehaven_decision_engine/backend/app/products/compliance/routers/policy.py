...
from app.products.compliance.services.policy_governance.notification_service import build_review_queue_entries, persist_review_queue_decision
...
from app.products.compliance.services.policy_assertions.validation_service import _source_url_validation_summary
...
def get_policy_profile_coverage_matrix(
    ...
    if profile.org_id is not None and profile.org_id != principal.org_id:
        raise HTTPException(status_code=403, detail="Forbidden")
    if recompute:
        from app.products.compliance.services.policy_coverage.completeness_service import recompute_profile_and_coverage
