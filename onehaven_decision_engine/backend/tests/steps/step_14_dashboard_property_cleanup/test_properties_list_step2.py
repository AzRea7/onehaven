...
def test_build_property_list_item_includes_step2_investor_fields(monkeypatch):
    ...
    assert row["crime_label"] == "MODERATE"
    assert row["normalized_decision"] == "GOOD"
    assert row["current_workflow_stage"] == "rehab"
    assert row["current_workflow_stage_label"] == "Rehab"
    assert row["gate_status"] == "BLOCKED"
    assert row["stage_completion_summary"]["completed_count"] == 1
    assert row["next_actions"] == ["Complete rehab tasks"]
    assert row["workflow"]["next_stage"] == "compliance"
