...
def auth_context(db_session):
    unique = uuid4().hex[:8]
    org_slug = f"step10-{unique}"
    user_email = f"step10-{unique}@demo.local"

    org = Organization(slug=org_slug, name=f"Step 10 Org {unique}")
    user = AppUser(email=user_email, display_name="Step 10 User")
