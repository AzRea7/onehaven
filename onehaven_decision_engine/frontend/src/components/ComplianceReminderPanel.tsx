...
function formatOffset(minutes?: number | null) {
  ...
  if (!value) return "Now";
  if (value % 1440 === 0) return `${value / 1440} day${value / 1440 === 1 ? "" : "s"} before`;
  if (value % 60 === 0) return `${value / 60} hour${value / 60 === 1 ? "" : "s"} before`;
...
export default function ComplianceReminderPanel({
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return rows.filter((row) => !row.property_id || row.property_id === propertyId);
  }, [propertyId, rows]);

  const sendReminder = async (row: ReminderRow) => {
    if (!row.inspection_id) return;
    try {
      setSendingId(row.inspection_id);
      const reminderOffsetMinutes =
        row.reminder_offset_minutes || row.due_offset_minutes || row.next_offset_minutes || 0;
  ...
  return (
    <Surface
      ...
      subtitle="Upcoming inspection reminders available for manual trigger or automation preview."
    >
      {loading ? (
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {row.status ? <span className="oh-pill">{row.status}</span> : null}
                  </div>
                ...
                <button
                  ...
                  disabled={!row.inspection_id || sendingId === row.inspection_id}
                  className="oh-btn oh-btn-secondary"
                >
                  <Send className="h-4 w-4" />
                  {sendingId === row.inspection_id ? "Sending..." : "Send reminder"}
                </button>
              </div>
            </div>
          ))}
