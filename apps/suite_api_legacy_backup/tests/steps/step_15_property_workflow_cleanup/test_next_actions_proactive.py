...
def test_property_ops_summary_exposes_jurisdiction_driven_next_actions(monkeypatch):
    prop = SimpleNamespace(
        id=45,
        org_id=1,
    ...
    monkeypatch.setattr(
        ...
        lambda db, org_id, property_id, recompute=True: {
            ...
            "normalized_decision": "GOOD",
            "gate_status": "BLOCKED",
            "gate": {"ok": False, "allowed_next_stage": "tenant"},
            "constraints": {
                "jurisdiction": {
                    "profile_id": 900,
            ...
            "outstanding_tasks": {
                "blockers": ["jurisdiction_incomplete", "jurisdiction_stale"]
            },
            "next_actions": [
                "Refresh jurisdiction profile for Warren.",
                "Verify certificate-before-occupancy rule coverage.",
            ],
            "stage_completion_summary": {"completed_count": 2, "total_count": 6},
        },
    )
    monkeypatch.setattr(
        ops_router,
        ...
        lambda db, org_id, property_id, recompute=False: {
            "current_stage": "compliance",
            "current_stage_label": "Compliance",
            "primary_action": {
                "kind": "next_action",
                "title": "Refresh jurisdiction profile for Warren.",
            },
        },
    )
    monkeypatch.setattr(
    ...
    result = ops_router.property_ops_summary(
        ...
        cash_days=90,
        db=FakeDB(),
        p=SimpleNamespace(org_id=1, user_id=100),
    )
    ...
    assert result["outstanding_tasks"]["blockers"] == [
