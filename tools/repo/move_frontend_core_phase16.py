...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Move high-confidence frontend app/platform/package files from Phase 15."
    )
    ...
    parser.add_argument(
        "--remove-legacy",
        action="store_true",
        help="Delete legacy files after successful copy. Do not use on first run.",
    )
...
def write_report(repo_root: Path, payload: dict) -> None:
    out = repo_root / "tools" / "repo" / "frontend-core-move-phase16-report.json"
...
def main() -> None:
    ...
    selected = [
        ...
        if record.get("bucket") in {"app", "platform", "package"}
    ...
    payload = {
        "phase": 16,
        "description": "Move high-confidence frontend app/platform/package files",
    ...
    print("Report written to tools/repo/frontend-core-move-phase16-report.json")
