...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Move product-owned backend files from Phase 7 report."
    )
...
def read_phase7(repo_root: Path) -> dict:
    return json.loads((repo_root / PHASE7_REPORT).read_text(encoding="utf-8"))
...
def write_report(repo_root: Path, payload: dict) -> None:
    out = repo_root / "tools/repo/product-owned-backend-phase11-report.json"
...
def main() -> None:
    ...
    payload = {
        "phase": 11,
        "description": "Move product-owned backend files",
    ...
    print("Report written to tools/repo/product-owned-backend-phase11-report.json")
