...
PHASE56_REPORT = "tools/repo/remaining-after-domain-phase56-report.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract top remaining legacy groups from Phase 56."
    )
    ...
    parser.add_argument(
        ...
        default="all",
        help="Filter groups by priority.",
    )
...
def main() -> None:
    ...
    report = read_json(repo_root / PHASE56_REPORT)
    ...
    groups = sorted(
        groups,
        key=lambda g: (
            g.get("priority_mix", {}).get(args.priority, 0)
            if args.priority != "all"
            else g.get("count", 0),
            g.get("count", 0),
        ),
    ...
    out = repo_root / "tools" / "repo" / f"remaining-groups-{args.priority}-phase57-report.json"
